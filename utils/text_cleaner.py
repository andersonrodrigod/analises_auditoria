...
extra_stop_words = """DE OU COM E UNILATERAL DO OARA SEM DA IMPLANTE PARA PROCEDIMENTOS TECNICA ORIENTADA QUALQUER PROGRAMA 24 MESES UMA MES COBRAR CORRESPONDENTE SEGMENTO SEGMENTAR TOTAL EM DEDICADO PERIFERICO TC SESSAO ARTICULAR VIDEOARTROSCOPICOS US CORONARIA TRATAMENTO QUANDO VIA PERCUTANEA TERAPEUTICA DIAGNOSTICA VIDEOENDOSCOPIA RETIRADA CORONARIANA QUIMIOTERAPICO ANTIANGIOGENICO RX CIRURGICO CIRURGICA VASO NERVO RM TURBINOPLASTIA ONCOLOGICO  INTRA CARTILAGINOSA VERTEBRAL A STENT TENDAO O DISCO CARDIACO PROCEDIMENTO BILATERAL FARMACOLOGICA TRANSLUMINAL 1 SISTEMA PROVAS COLOCACAO CIRURGIA CIRCUITO PARCIAL LIGAMENTO INCLUI RADIOFREQUENCIA BALAO SENSIBILIZACAO ARRITMOGENICO  VEIAS INTRODUCAO MULTIPOLAR NAS CODIGO VIDEOLAPAROSCOPIA LAPAROSCOPICA CATETER SEPTOPLASTIA ENDOSCOPICA CORRECAO TUNEL OMBRO ADENOIDECTOMIA URETER REFORCO COXOFEMORAL ANESTESIA METODO FACETARIA PUNHO CARPO FACETAS RETENCIONAMENTO MULTIPLOS VASOS BIFURCACAO IMPACTO FEMORO ACETABULAR OPERATORIA INFUSAO INTRAVITREA MEDICAMENTO ANTI INFLAMATORIO TRIDIMENSIONAL AMBIENTE LABRAL ANTERIOR DRENAGEM ORIENTADO ARTICULARES HOSPITALAR IMAGEM SUTURA TRIGEMIO LOMBAR MONITORIZACAO LATERAL RECIDIVANTE CRUZADO TEMPORO TEMPORO ARTICULACAO NEUROFISIOLOGICA MANDIBULAR AO NIVEL GERADOR MENISCO REPARO TRANSPOSICAO VIDEOTORACOSCOPIA PLEUROSCOPIA VIDEO PLEURAL FECHADA ALTA C INSTRUMENTACAO NERVOSO SINEQUIAS UNICO FETO MULTIPLO IMPLANTES PERIDURAL SUBARACNOIDEO ESPLENECTOMIA CORTICOIDE APARELHO EXTENSOR REALINHAMENTOS POS CONTROLE CLAVICULA GERAL DOS DEDOS COXA FÊMUR ABDOME AVENTAL EXPLORADORA RETALHOS POSTERIOR CANAL ESTREITO CAUDA EQUINA MAIS LIGAMENTARES EXTERNA TUBO VENTILACAO CORPOS INSTALACAO DO CIRCUITO DE CIRCULACAO EXTRACORPOREA CONVENCIONAL PROGNATISMO MICROGNATISMO OPERACAO DE PROCESSADORA AUTOMATICA DE SANGUE EM AUTOTRANSFUSAO INTRA/OPERATORIA VERTEBROPLASTIA OUTRAS CXL COLAGENO CORNEANO PERFUSIONISTA CARDIACA E VIA  OVARIANAS VARICOCEL TRONCO VENOSO VASCULAR RETALHO COMPOSTO INCLUINDO CARTILAGEM OSSO VARICOCELE EXPLORACAO CIRURGICA DE NERVO ELETRODOS ESTIMULACAO CEREBRAL INTRAVASCULAR TRANSOPERATORIA POSICIONAMENTO NEUROLISE EXTERNA RADIOSCOPIA ACOMPANHAMENTO HORA FRACAO MICROCIRURGICO DISSECCAO VEIA NEUROLITICO CENTRAL AUTONOMO GALEA APONEUROTICA GRAVIDADE OBSTRUCOES GUIA INTERNA FLEXIVEL MASTOIDECTOMIA TRIANGULAR NARIZ ANIPULACAO SOB OUTROS PUNCOES MICROCIRURGIA TRANSESFENOIDAL ESTABILIZACAO PLASTIA CAVO COALISAO TARSAL LIQUORICA COCLEAR EXCETO PROTESE VERSAO INTRAVITREO POLIMERO FARMACOLOGICO LIBERACAO MANIPULACAO MENISCORRAFIA ANEURISMA  SACULAR CONCOMITANTE CORONARIO ETMOIDECTOMIA INTRANASAL ESTEREOTAXIA ENXERTIA JOELHO URETEROSCOPICA DO SISTEMA DE CONDUCAO COM OU SEM ACAO FARMACOLOGICA HEMATOMA PANARICIO POR NAO EXAME MANUTENCAO ESTERILIZADORA MASCULINA OBESIDADE MORBIDA NO OSTEO FIBROSO CAUTERIZACAO LINEAR REMOCAO LIVRES""".split()
...
def normalize_words(df, column="word"):
    ...
    def mapper(word):
        ...
        if "CATETERIZACAO TRANSEPTAL" in word_upper:
            return "CAT/TRANS"
        if "LESOES INTRINSECAS" in word_upper or "JOELHO" in word_upper:
            return "LES_INT_JOELHO"
        if "COLECTOMIA" in word_upper:
            return "COLECTOMIA"
        if "LINFADENECTOMIA" in word_upper:
            return "LINFADENECTOMIA"
        if "ATEROMECTOMIA" in word_upper:
            return "ATEROMECTOMIA"
        if "MAPEAMENTO ELETROANATOMICO" in word_upper:
            return "MAP/ELETRONAMICO"
        if "HISTEROSCOPIA" in word_upper:
            return "HISTEROSCOPIA"
        if "RESSECCAO TUMOR" in word_upper:
            return "RES/TUMOR"
        if "ANGIOGRAFIA CATETERISMO" in word_upper:
            return "ANGIO/CATE"
        if "RECONSTRUCAO SULCO GENGIVO LABIAL" in word_upper:
            return "RECONS/SULC/GENG"
        if "CATETERISMO" in word_upper:
            return "CATETERISMO"
        if "RETOSSIGMOIDECTOMIA" in word_upper:
            return "RETOSSIG."
        if "NEFROURETERECTOMIA" in word_upper:
            return "NEFROURET."
        if "AVALIACAO FISIOLOGIC" in word_upper:
            return "AVAL/FISIO."
        if "NEVRALGIA" in word_upper:
            return "TRAT/NEVRALGIA"
        if "OCLUSAO SHUNTS" in word_upper:
            return "OCLUSAO/PERC"
        if "QUADRANTECTOMIA" in word_upper:
            return "QUADRANTECT"
        if "RECONSTRUCAO ROTACAO MUSCULO" in word_upper:
            return "RECONS/ROT/MUSC"
        if "HIPERATIVIDADE VESICAL" in word_upper:
            return "TRAT/HIPERATI"
        if "PARACENTESE TIMPANO" in word_upper:
            return "PARACENTESE/TIP"
        if "NEFROLITOTRIPSIA" in word_upper:
            return "NEFROLI."
        if "NEUROTOMIA" in word_upper:
            return "NEUROTOMIA"
        if "NEUROESTIMULACAO" in word_upper:
            return "NEUROESTIMULACAO"
        if "DESFIBRILADOR INTERNO" in word_upper:
            return "IMP/DESFI/INT"
        if "ANGIOPLASTIA" in word_upper:
            return "ANGIOPLASTIA"
        if "LONGA PERMANENCIA NPP" in word_upper:
            return "IMP/CATETER"
        if "OXIGENOTERAPIA" in word_upper:
            return "OXIGENOTERAPIA"
        if "SINUSECTOMIA" in word_upper:
            return "SINUSECTOMIA"
        if "GASTROPLATIA" in word_upper:
            return "GASTROPLATIA"
        if "ESTELIZADORA MASCULINA" in word_upper:
            return "VASECTOMIA" 
        if "PATELO FEMORAL" in word_upper:
            return "PATELO/FEMORAL"
        if "ARTROTOMIA GLENOUMERAL" in word_upper:
            return "ARTOTROMIA/GLENO."
        ...
        if "EMBOLIZACAO" in word_upper or "MEDULAR" in word_upper:
            return "EMBOLIZACAO"
        if "ARTROSCOPIA" in word_upper or "BIOPSIA" in word_upper:
            return "ARTRO/DIAG/BIOPSIA"
